...
On commence par installer les pré-requis puis on en profite pour faire un mise à jour :
```shell
$ apt-get update && apt-get upgrade && apt-get install apt-transport-https ca-certificates xz-utils iptables aufs-tools git
```

Puis on ajoute le dépot, ainsi que le clé GPG de celui ci :
```shell
...
Il existe plusieurs version de docker dans ce dépot, vous pouvez choisir celle ci :
```shell
...
Il ne nous reste plus qu'a lancer docker :
```shell
...
Pour ce faire, clique droit sur le menu d'application -> panneau de configuration -> Programmes et fonctionnalités -> Activer ou désactiver des fonctionnalités Windows -> Coche Hyper-V -> OK.

On redémarre le PC, et normalement c'est bon, nous avons Hyper-V.

On peux passer à l'installation de docker4windows, et la c'est vraiment simple, télécharger l'exécutable [ici](https://download.docker.com/win/beta/InstallDocker.msi), ensuite c'est du next-next.
...
On peut désormais tester, on ouvre un console (touche windows+R puis cmd), il faut se positionner dans le C:\docker\bin, puis on test la connexion (penser à changer l'IP par celle de votre serveur docker) :
