...
* **host** : Ce type de réseau permets au conteneurs d'accéder directemment à votre réseau.
...
```
Comme nous pouvons le voir, le réseau à comme subnet 172.18.0.0/16, et comme gateway 172.18.0.1. Les conteneurs aurons donc des IP d'attribué entre 172.18.0.2 et 172.18.255.254 (172.18.255.255 étant le broadcast).


Nous pouvons évidemment choisir ce subnet, les IP à attribué au conteneurs etc ...
...
```shell$ docker run -ti --network test3 --name ctest alpine:3.4 sh
/ # ifconfig
...
```
Puis revener dans le conteneur :
```shell
/ # ifconfig
eth0      Link encap:Ethernet  HWaddr 02:42:C0:A8:C8:02
          inet addr:192.168.200.2  Bcast:0.0.0.0  Mask:255.255.255.0
...
L'un des avantages des networks, c'est qu'il n'est plus nécessaire de créer des liens entre les conteneurs.
